/**
  ...
  showBrowserLoginForm() {
    ...
    // Добавляем стили
    if (!document.getElementById('browser-auth-styles')) {
    ...
    document.getElementById('browser-auth-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      ...
      try {
        await this.authenticateWithPassword(password, { name });
      } catch (error) {
        // Показываем ошибку
        errorDiv.innerHTML = `
